...
function ConfigurationsPage() {
  ...
  useEffect(() => { setItems(savedConfigs.list()); }, []);

  return (
    <div className="mx-auto w-full max-w-5xl px-6 py-8">
      <h1 className="text-2xl font-semibold tracking-tight">Saved configurations</h1>
      <p className="mt-1 text-sm text-muted-foreground">Configurations are stored locally in this browser.</p>

      {items.length === 0 ? (
        <div className="panel-surface mt-8 rounded-lg p-10 text-center text-sm text-muted-foreground">
          No saved configurations yet. Open a product in the studio and press <span className="font-mono">Save</span>.
        </div>
      ) : (
        <ul className="mt-6 grid gap-3">
          {items.map((c) => {
            ...
            return (
              <li key={c.id} className="panel-surface flex items-center justify-between rounded-lg p-4">
                <div>
                  <p className="font-medium">{c.name}</p>
                  <p className="mt-1 font-mono text-xs text-muted-foreground">
                    {new Date(c.updatedAt).toLocaleString()} · source {parsed?.sourceProductId ?? "—"}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => { savedConfigs.remove(c.id); setItems(savedConfigs.list()); }}
