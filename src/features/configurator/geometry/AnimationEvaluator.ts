...
function rotationAngle(operation: { angleRadians?: number; angleDegrees?: number }): number {
...
export function evaluatePackagingAnimation(
  ...
  sequence.steps.forEach((step, stepPosition) => {
    ...
    for (const operation of step.operations) {
      ...
      if (!panelId) continue;
      const current = panelTransforms.get(panelId) ?? {
        ...
        const vector = operation.vector
          ...
          : operation.axis
            ? new THREE.Vector3(...operation.axis)
                .normalize()
                .multiplyScalar(operation.distance ?? 0)
            : new THREE.Vector3();
