  ...
  const doExport = async () => {
    ...
    try {
      if (format === "json") {
        ...
        toast.success(t("export.success.dieline"));
      } else if (format === "png") {
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="grid grid-cols-3 gap-2">
          <FormatBtn
            ...
            label={t("export.format.png")}
          />
          <FormatBtn
            active={format === "svg"}
            onClick={() => setFormat("svg")}
            icon={<FileCode className="h-4 w-4" />}
            label={t("export.format.svg")}
          />
          <FormatBtn
            active={format === "json"}
...
function FormatBtn({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-1 rounded-md border p-3 text-xs ${active ? "border-gold bg-accent" : "border-input hover:bg-accent/60"}`}
    >
      {icon}
      {label}
    </button>
...
function buildDielineSvg(
  ...
  >["dieline"] extends infer T
    ? T extends { totalX: number }
      ? T
      : never
    : never,
  transparent: boolean,
  ...
  const foldsSvg = folds
    .map(
      (f) =>
        `<line x1="${f.from[0]}" y1="${f.from[1]}" x2="${f.to[0]}" y2="${f.to[1]}" stroke="#3366cc" stroke-width="0.5" stroke-dasharray="3 2"/>`,
    )
    .join("");
  const labels = faces
    .map(
      (p) =>
        `<text x="${p.centroid.x}" y="${p.centroid.y}" text-anchor="middle" font-size="6" fill="#888" font-family="monospace">${p.name}</text>`,
    )
    .join("");
  const arrows = sizeArrows
    .map(
      (s) =>
        `<line x1="${s.p1.x}" y1="${s.p1.y}" x2="${s.p2.x}" y2="${s.p2.y}" stroke="#666" stroke-width="0.3"/>${s.tpointer ? `<text x="${s.tpointer.x}" y="${s.tpointer.y}" text-anchor="middle" font-size="6" font-family="monospace">${s.label}</text>` : ""}`,
    )
    .join("");
  const bleeds = bleedsPath
    ? `<path d="${bleedsPath}" fill="none" stroke="#cc00cc" stroke-width="0.4" stroke-dasharray="4 2"/>`
    : "";
