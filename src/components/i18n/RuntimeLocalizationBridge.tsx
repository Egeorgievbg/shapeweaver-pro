...
function localizeTextNode(node: Text, locale: "bg" | "en") {
  ...
  if (!trimmed) return;
  const translated = locale === "bg" ? translateRuntimeText(trimmed, "bg") : trimmed;
...
function localizeElementAttributes(element: Element, locale: "bg" | "en") {
  ...
  for (const attribute of LOCALIZED_ATTRIBUTES) {
    ...
    const next = locale === "bg" ? translateRuntimeText(base, "bg") : base;
...
function localizeTree(root: Node, locale: "bg" | "en") {
  ...
  while (node) {
    ...
    if (node instanceof Element) localizeElementAttributes(node, locale);
    node = walker.nextNode();
  }
}

/**
 * Compatibility layer for legacy hard-coded interface labels.
 * New screens must use keyed translations through useI18n(). This bridge keeps
 * older dynamically rendered dialogs and editor controls consistently bilingual
 * while they are migrated incrementally.
 */
export function RuntimeLocalizationBridge() {
